  ...
  try {
    ...
    const keyStatsResult = await db
      .select({
        ...
        upcomingTrips: sql<number>`COUNT(*) FILTER (WHERE ${
          trips.startDate
        } > ${now.toISOString()} AND ${
          trips.status
        } IN ('active','pending'))::int`,
    ...
    const tripCategories = categoriesResult.map((c) => ({
      name: c.type || "Other",
      count: c.count,
      percentage: totalForCategories
        ? Math.round((c.count / totalForCategories) * 100)
        : 0,
    }));
    ...
    const recentActivity = await db
      ...
      .innerJoin(tripCoordinators, eq(tripCoordinators.tripId, trips.id))
      .where(
        sql`${tripCoordinators.userId} = ${coordinatorId} AND ${trips.updatedAt} >= ${sevenDaysAgo}`
      )
        ...
        activeTrips: {
          ...
          percentageChange: parseFloat(
            calcPercentageChange(
              keyStats.activeCurrentMonth,
              keyStats.activePreviousMonth
            ).toFixed(1)
          ),
          isPositive:
            keyStats.activeCurrentMonth >= keyStats.activePreviousMonth,
        },
