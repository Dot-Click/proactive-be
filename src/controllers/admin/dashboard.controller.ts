...
export const dashboardlogic = async (_req: Request, res: Response) => {
  try {
    ...
    const previousMonthCoordinators =
      Number(previousMonthCoordinatorsResult[0]?.count) || 0;
    const coordinatorsPercentageChange = calculatePercentageChange(
      coordinators,
      previousMonthCoordinators
    );
    ...
    const previousMonthActiveTripsResult = await db
      ...
      .from(trips)
      .where(
        and(eq(trips.status, "active"), lte(trips.createdAt, previousMonthEnd))
      );
    const previousMonthActiveTrips =
      Number(previousMonthActiveTripsResult[0]?.count) || 0;
    const activeTripsPercentageChange = calculatePercentageChange(
      activeTrips,
      previousMonthActiveTrips
    );
    ...
    const previousMonthClosedTrips =
      Number(previousMonthClosedTripsResult[0]?.count) || 0;
    const closedTripsPercentageChange = calculatePercentageChange(
      closedTrips,
      previousMonthClosedTrips
    );
    ...
    const currentMonthEarnings =
      Number(currentMonthEarningsResult[0]?.total) || 0;
    ...
    const previousMonthEarnings =
      Number(previousMonthEarningsResult[0]?.total) || 0;
    const earningsPercentageChange = calculatePercentageChange(
    ...
    const recentActivity = recentActivitiesResult.map((activity) => {
      ...
      return {
        id: activity.id,
        message: `User ${userName} upgraded to ${
          activity.membershipType || "Gold"
        } Membership`,
        timeAgo: timeAgo,
